...
module.exports.config = {
  ...
  role: 0,
  author: "Satoru",
  info: "Làm nét ảnh bằng AI (Có thể dùng URL)",
  category: "Công cụ",
  cd: 5,
  Prefix: true
};
this.onCall = async function ({ api, event, args }) {
  ...
  api.sendMessage("⏳ Đang làm nét ảnh ...", event.threadID, async (err, info) => {
    try {
      ...
      const resultResponse = await axios.get(buffer.result.image_url, { responseType: "arraybuffer" });
      fs.writeFileSync(pathSave, resultResponse.data);
      return api.sendMessage(
        {
...
function reminiUpscale(buffer) {
  return new Promise((resolve, reject) => {
    ...
    form.append("original_image_file", buffer, { filename: "image.png", contentType: "image/png" });
    const productSerial = generateFingerprint();
    axios.post("https://api.remaker.ai/api/pai/v4/ai-enhance/create-job-new", form, {
      headers: {
        ...form.getHeaders(),
        "Authorization": "",  // Thêm token vào đây nếu cần
        "Product-Code": "067003",
