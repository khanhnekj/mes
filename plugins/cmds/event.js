...
this.loadCommand = function ({ moduleList, threadID, messageID }) {
    ...
    for (const nameModule of moduleList) {
        try {
            ...
            if (!event.config || !event.onCall) throw new Error("❎ Định dạng lỗi.");
            if (event.config.envConfig && typeof event.config.envConfig == "object") {
                try {
                    ...
                    logger(`Đã tải thành công config cho module ${event.config.name}`);
                }
            ...
            logger(`Đã tải sự kiện ${event.config.name}!`);
...
this.onCall = function ({ event, args, api }) {
