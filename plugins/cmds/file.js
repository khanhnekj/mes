...
this.onCall = function({ api, event, args, commandName }) {
    openFolder(api, event, process.cwd() + (args[0] ? args[0] : ''), commandName);
};

this.onReply = function({ Reply: $, api, event, commandName }) {
    try {
        ...
        switch (action) {
            case 'open':
                if (d.info.isDirectory()) openFolder(api, event, d.dest, commandName);
...
function openFolder(a, b, c, commandName) {
    ...
    a.sendMessage(txt, b.threadID, (err, data) => global.Seiko.onReply.set(data.messageID, {
        commandName,
