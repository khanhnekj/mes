...
async function getHighlight(url, cookie) {
  try {
    ...
    const resp = data.find(item => item?.pk === storyId) || data.find(item => item?.id === storyId);
    let attachments = [];
...
async function post(username) {
  ...
  const res = await axios.get(`https://i.instagram.com/api/v1/feed/user/${id}/`, {
    headers
  });
  delete headers.cookie;
  const data = res.data.items;
  ...
  const results = data.map(item => {
    ...
    if (info.video_versions) {
      ...
      dataReturn.images = allImage.map(mediaItem => mediaItem.image_versions2.candidates[0].url);
    }
    ...
    return {
      id: postData.originalData.id,
      message: postData.caption || null,
      author: `${postData.owner.full_name} (${postData.owner.username})` || null,
      like: formatNumber(postData.like_count) || null,
      comment: formatNumber(postData.comment_count) || null,
      play: formatNumber(postData.originalData.play_count) || null,
      attachments
